...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cnp-bot-test-'));
}

export function setupNanoclawDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.cnp-bot', 'base', 'src'), { recursive: true });
  fs.mkdirSync(path.join(tmpDir, '.cnp-bot', 'backup'), { recursive: true });
}

export function writeState(tmpDir: string, state: any): void {
  const statePath = path.join(tmpDir, '.cnp-bot', 'state.yaml');
