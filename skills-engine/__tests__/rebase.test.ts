...
describe('rebase', () => {
  ...
  it('rebase with one skill: patch created, state updated, rebased_at set', async () => {
    // Set up base file
    const baseDir = path.join(tmpDir, '.cnp-bot', 'base', 'src');
    ...
    const patchPath = path.join(tmpDir, '.cnp-bot', 'combined.patch');
    ...
    const stateContent = fs.readFileSync(
      path.join(tmpDir, '.cnp-bot', 'state.yaml'),
      'utf-8',
    );
  ...
  it('rebase flattens: base updated to match working tree', async () => {
    ...
    const baseContent = fs.readFileSync(
      path.join(tmpDir, '.cnp-bot', 'base', 'src', 'index.ts'),
      'utf-8',
    );
  ...
  it('rebase with multiple skills + custom mods: all collapsed into single patch', async () => {
    // Set up base files
    const baseDir = path.join(tmpDir, '.cnp-bot', 'base');
      ...
      custom_modifications: [
        {
          ...
          patch_file: '.cnp-bot/custom/001-tweaked-config.patch',
        },
      ],
    });
    ...
    const patchContent = fs.readFileSync(
      path.join(tmpDir, '.cnp-bot', 'combined.patch'),
      'utf-8',
    );
    ...
    const baseConfig = fs.readFileSync(
      path.join(tmpDir, '.cnp-bot', 'base', 'src', 'config.ts'),
      'utf-8',
    );
