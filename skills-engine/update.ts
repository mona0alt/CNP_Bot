...
import { BASE_DIR, CNP_BOT_DIR } from './constants.js';
  ...
  try {
    ...
    for (const relPath of preview.filesChanged) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `cnp-bot-update-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
    ...
    const remapFile = path.join(newCorePath, '.cnp-bot-meta', 'path_remap.yaml');
