...
### PAAS 数据源网段分布 (http://thanos.paas.gwm.cn)

| 网段 | 区域 | 环境 | 节点数 |
|------|------|------|--------|
| 10.245.16.x | baoding (保定) | 生产环境 | - |
| 10.246.7.x | baoding (保定) | 生产环境 | - |
| 10.255.23.x | baoding (保定) | 测试/预发布/生产环境 | 60 |
| 10.255.30.x | baoding (保定) | 测试环境 | - |
| 10.255.67.x | baoding (保定) | 测试环境 | - |
| 10.255.128.x | baoding (保定) | 测试环境 | - |
| 10.255.131.x | baoding (保定) | 预发布环境 | - |
| 10.136.130.x | thailand (泰国) | 生产环境 | - |
| 10.35.131.x | chongqing (重庆) | 生产环境 | - |
| 10.38.13.x | yangzhong (扬中) | 生产环境 | - |
| 10.38.14.x | yangzhong (扬中) | 生产环境 | - |
| 10.55.129.x | pinghu (平湖) | 生产环境 | - |
| 10.70.118.x | daye (大冶) | 生产环境 | - |
| 172.30.x.x | baoding (保定) | 测试环境 (K8s Pod) | 3 |

#### 按区域查询示例 (PAAS)

```bash
# 查询保定测试环境节点
node scripts/cli.js query 'up{region="baoding",env_zh="测试环境"}'

# 查询保定生产环境节点
node scripts/cli.js query 'up{region="baoding",env_zh="生产环境"}'

# 查询大冶生产环境节点
node scripts/cli.js query 'up{region="daye",env_zh="生产环境"}'

# 查询泰国生产环境节点
node scripts/cli.js query 'up{region="thailand",env_zh="生产环境"}'
```
...
配置文件保存在 skill 目录下：`~/.claude/skills/prometheus/prometheus.json`
...
2. Skill 目录: `~/.claude/skills/prometheus/prometheus.json` **(推荐)**
3. `~/.openclaw/workspace/prometheus.json`
4. `~/.openclaw/workspace/config/prometheus.json`
5. `./prometheus.json` (current directory)
6. `~/.config/prometheus/config.json`

### Config Format

创建 `prometheus.json` 在 skill 目录下 (或使用 `node cli.js init`):
...
- Config is stored in skill directory (`~/.claude/skills/prometheus/prometheus.json`)
