...
const chatJid = chatJidFlag !== -1 ? args[chatJidFlag + 1] : (process.env.CNP_BOT_CHAT_JID || '');
...
function runChart(args, cwd = SKILL_DIR) {
  const result = spawnSync('node', ['scripts/chart.js', ...args], {
    ...
    encoding: 'utf8',
    timeout: 30000,
    env: { ...process.env, CNP_BOT_CHAT_JID: chatJid },
  });
  return {
    stdout: result.stdout || '',
    stderr: result.stderr || '',
...
await testAsync('chart.js - 节点 CPU 图表（portal，保定管理环境节点）', async () => {
  if (!chatJid) throw new Error('需要 --chat-jid 参数或 $CNP_BOT_CHAT_JID 环境变量');
