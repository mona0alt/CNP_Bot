...
<p align="center">
  CNP-Bot —— 您的专属 Claude 助手，在容器中安全运行。它轻巧易懂，并能根据您的个人需求灵活定制。
</p>
...
CNP-Bot 用一个您能在 8 分钟内理解的代码库，为您提供了同样的核心功能。只有一个进程，少数几个文件。智能体（Agent）运行在具有文件系统隔离的真实 Linux 容器中，而不是依赖于权限检查。
...
- 管理员可在 `/settings` 查看和编辑系统级配置。
- 所有设置最终都会写回项目根目录下的 `.env`，它仍然是运行时配置的唯一真源。
- 页面同时提供“保存配置”和“保存并重启”，可直接在 Web 管理端完成生效流程。
- 修改 `JWT_SECRET` 后，现有登录会话会失效，服务重启后需要重新登录。
...
如果您想添加 Telegram 支持，不要创建一个 PR 同时添加 Telegram 和 WhatsApp。而是贡献一个技能文件 (`.claude/skills/add-telegram/SKILL.md`)，教 Claude Code 如何改造一个 CNP-Bot 安装以使用 Telegram。
