...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.cnp-bot')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.cnp-bot'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active cnp-bot`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
        const output = execSync(`${prefix} list-unit-files`, { encoding: 'utf-8' });
        if (output.includes('cnp-bot')) {
          service = 'stopped';
        }
      } catch {
  ...
  if (fs.existsSync(path.join(homeDir, '.config', 'cnp-bot', 'mount-allowlist.json'))) {
